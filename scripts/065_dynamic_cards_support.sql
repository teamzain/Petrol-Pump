-- Multi-Payment Support v3: Dynamic Card Types
-- This script adds JSONB support for dynamic card breakdowns and updates the financial trigger.

-- 1. Add new columns to nozzle_readings and sales
ALTER TABLE public.nozzle_readings ADD COLUMN IF NOT EXISTS total_card_amount DECIMAL(15,2) DEFAULT 0;
ALTER TABLE public.nozzle_readings ADD COLUMN IF NOT EXISTS card_breakdown JSONB DEFAULT '{}';

ALTER TABLE public.sales ADD COLUMN IF NOT EXISTS total_card_amount DECIMAL(15,2) DEFAULT 0;
ALTER TABLE public.sales ADD COLUMN IF NOT EXISTS card_breakdown JSONB DEFAULT '{}';

-- 2. Create the V3 Trigger Function
...
BEGIN
    ...
    IF (TG_OP = 'DELETE' OR TG_OP = 'UPDATE') THEN
        ...
        -- Use total_card_amount if available, fallback to legacy shell+bank for migration compatibility
        v_old_total_card := COALESCE(OLD.total_card_amount, COALESCE(OLD.shell_card_amount, 0) + COALESCE(OLD.bank_card_amount, 0));
    ...
    IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
        ...
        -- Recalculate total_card from JSONB if provided, otherwise use the column
        IF NEW.card_breakdown IS NOT NULL AND NEW.card_breakdown != '{}'::jsonb THEN
            SELECT SUM(value::text::decimal) INTO v_total_card 
            FROM jsonb_each(NEW.card_breakdown);
        ELSE
            v_total_card := COALESCE(NEW.total_card_amount, COALESCE(NEW.shell_card_amount, 0) + COALESCE(NEW.bank_card_amount, 0));
        END IF;
        ...
        IF NEW.card_breakdown IS NOT NULL AND NEW.card_breakdown != '{}'::jsonb THEN
            ...
            END LOOP;
        ELSE
            -- Legacy Fallback (if they still send shell_card_amount/bank_card_amount)
            IF COALESCE(NEW.shell_card_amount, 0) > 0 THEN
                ...
                IF v_card_type_id IS NOT NULL THEN
                    INSERT INTO public.card_payments (card_type_id, reference_type, reference_id, amount, tax_percentage, tax_amount, net_amount, status, created_by, payment_date)
                    VALUES (v_card_type_id, TG_TABLE_NAME, NEW.id, NEW.shell_card_amount, v_tax_pct, (NEW.shell_card_amount * v_tax_pct / 100), NEW.shell_card_amount - (NEW.shell_card_amount * v_tax_pct / 100), 'hold', v_recorded_by, v_date);
                END IF;
            END IF;
            IF COALESCE(NEW.bank_card_amount, 0) > 0 THEN
                SELECT id, tax_percentage INTO v_card_type_id, v_tax_pct FROM public.card_types WHERE card_name = 'Bank Card' LIMIT 1;
                IF v_card_type_id IS NOT NULL THEN
                    INSERT INTO public.card_payments (card_type_id, reference_type, reference_id, amount, tax_percentage, tax_amount, net_amount, status, created_by, payment_date)
                    VALUES (v_card_type_id, TG_TABLE_NAME, NEW.id, NEW.bank_card_amount, v_tax_pct, (NEW.bank_card_amount * v_tax_pct / 100), NEW.bank_card_amount - (NEW.bank_card_amount * v_tax_pct / 100), 'hold', v_recorded_by, v_date);
                END IF;
            END IF;
        END IF;
...
-- 4. Re-bind triggers to V3
DROP TRIGGER IF EXISTS tr_nozzle_readings_financials ON public.nozzle_readings;
CREATE TRIGGER tr_nozzle_readings_financials
AFTER INSERT OR UPDATE OR DELETE ON public.nozzle_readings
FOR EACH ROW EXECUTE FUNCTION public.handle_sale_financials_v3();

DROP TRIGGER IF EXISTS tr_sales_financials ON public.sales;
CREATE TRIGGER tr_sales_financials
