-- 100_fix_purchase_trigger_and_data.sql
-- Run this to fix "Missing Account" columns and ensure future purchases are correct.

BEGIN;

-- 1. INSTALL THE ROBUST PURCHASE ORDER TRIGGER (Ensures future IDs are set)
...
BEGIN
    ...
    IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
        ...
        INSERT INTO public.transactions (
            ...
            v_account_id, -- Ensures this is set
            'purchase_order', 
            NEW.id, 
            v_account_id,
            NEW.created_by
...
SET from_account = (SELECT id FROM public.accounts WHERE account_type = 'cash' AND status = 'active' LIMIT 1),
    bank_account_id = (SELECT id FROM public.accounts WHERE account_type = 'cash' AND status = 'active' LIMIT 1)
