-- 101_fix_purchase_trigger_syntax_and_data.sql
-- FIXES SYNTAX ERROR: "ORDER BY" clause in UPDATE statement is not allowed directly.

BEGIN;

-- 1. INSTALL THE ROBUST PURCHASE ORDER TRIGGER (With Corrected Syntax)
...
BEGIN
    ...
    IF (TG_OP = 'DELETE' OR TG_OP = 'UPDATE') THEN
        ...
        ELSIF OLD.payment_method = 'cash' THEN
            -- FIXED SYNTAX: Use subquery for ID
...
-- 3. DATA REPAIR: FIX EXISTING MISSING ACCOUNTS
-- Update empty from_account for Cash transactions
UPDATE public.transactions 
SET from_account = (SELECT id FROM public.accounts WHERE account_type = 'cash' AND status = 'active' ORDER BY created_at ASC LIMIT 1),
    bank_account_id = (SELECT id FROM public.accounts WHERE account_type = 'cash' AND status = 'active' ORDER BY created_at ASC LIMIT 1)
WHERE (from_account IS NULL OR bank_account_id IS NULL)
AND payment_method = 'cash'
AND (reference_type = 'purchase' OR reference_type = 'purchase_order');

-- Update empty from_account for Bank transactions
UPDATE public.transactions 
SET from_account = (SELECT id FROM public.accounts WHERE account_type = 'bank' AND status = 'active' ORDER BY created_at ASC LIMIT 1),
    bank_account_id = (SELECT id FROM public.accounts WHERE account_type = 'bank' AND status = 'active' ORDER BY created_at ASC LIMIT 1)
WHERE (from_account IS NULL OR bank_account_id IS NULL)
AND payment_method IN ('bank', 'bank_transfer', 'cheque')
AND (reference_type = 'purchase' OR reference_type = 'purchase_order');
