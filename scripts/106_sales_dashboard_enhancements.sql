-- 106_sales_dashboard_enhancements.sql
-- 1. Add total_amount (retail) and paid_amount (actual) to sales table
ALTER TABLE public.sales 
ADD COLUMN IF NOT EXISTS total_amount DECIMAL(15, 2),
ADD COLUMN IF NOT EXISTS paid_amount DECIMAL(15, 2);

-- 2. Backfill existing records: sale_amount is current truth
UPDATE public.sales 
SET total_amount = sale_amount,
    paid_amount = sale_amount
WHERE total_amount IS NULL;

-- 3. Update the handle_sale_financials trigger to be aware of paid_amount
-- We prioritize paid_amount if it's NOT NULL, otherwise fallback to sale_amount (for fuel sales/compatibility)
...
BEGIN
    ...
    IF (TG_OP = 'DELETE' OR TG_OP = 'UPDATE') THEN
        -- Use paid_amount if exists (Product Sales), else sale_amount (Fuel/Old records)
        IF TG_TABLE_NAME = 'sales' THEN
            v_old_amount := COALESCE(OLD.paid_amount, OLD.sale_amount);
        ELSE
    ...
    -- 3. APPLY NEW IMPACT
    IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
        -- Use paid_amount if exists, else sale_amount
        IF TG_TABLE_NAME = 'sales' THEN
            v_amount := COALESCE(NEW.paid_amount, NEW.sale_amount);
        ELSE
